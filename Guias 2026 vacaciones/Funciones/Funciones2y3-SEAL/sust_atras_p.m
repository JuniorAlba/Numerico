function [x] = sust_atras_p(A)
    ...
    % (Usada generalmente por 'gauss_p' con matriz aumentada).
    %
    % Entradas:
    ...
    if (opcion_warnings && abs(A(n, n)) < TOL_PIVOTE)
        printf("ADVERTENCIA (sust_atras_p): Pivote A(%d,%d) es cercano a cero.\n", n, n);
    endif
    x(n) = A(n, n + 1) / A(n, n);
    ...
    if (medir_tiempo)
        ...
        if (opcion_verbose >= 1)
            printf("Sustitución Hacia Atrás (Pivoteo Vec) finalizada. Tiempo: %f s\n", tiempo_ejecucion);
        endif
    endif
...
% Sustitución Hacia Atrás (Pivoteo Vec) finalizada. Tiempo: ... s
% x =
%
%    0.5000
%    0.0000
